...
def process_data(rows, dimension, exclude_blog=False):
    # Convert rows to DataFrame
    data = []
    for row in rows:
        key = row["keys"][0]
        if exclude_blog and "/blog" in key:
            continue
        data.append(
            {
                "key": key,
...
# Fetch and Process Data for Pages, excluding '/blog'
...
df_pages_1 = process_data(first_period_pages, "page", exclude_blog=True)
df_pages_2 = process_data(second_period_pages, "page", exclude_blog=True)

# Compare Data for Queries
gained_impressions_queries, lost_impressions_queries = compare_data(
    df_queries_1, df_queries_2, "impressions"
)
gained_clicks_queries, lost_clicks_queries = compare_data(
    df_queries_1, df_queries_2, "clicks"
)

# Compare Data for Pages
gained_impressions_pages, lost_impressions_pages = compare_data(
    df_pages_1, df_pages_2, "impressions"
)
...
results = pd.concat(
    [
        gained_impressions_queries,
        lost_impressions_queries,
        gained_clicks_queries,
        lost_clicks_queries,
        gained_impressions_pages,
        lost_impressions_pages,
        gained_clicks_pages,
        lost_clicks_pages,
    ],
    keys=[
        "Gained Impressions Queries",
        "Lost Impressions Queries",
        "Gained Clicks Queries",
        "Lost Clicks Queries",
        "Gained Impressions Pages",
        "Lost Impressions Pages",
        "Gained Clicks Pages",
        "Lost Clicks Pages",
    ],
)
results.to_csv("gsc-pagesPT.csv")

print("Comparison results saved to 'gsc-pagesPT.csv'")
