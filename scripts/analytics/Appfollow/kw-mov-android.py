...
# Configuration
ext_id_android = 'com.kyte.catalog'
device = 'android'
countries = ['br', 'us', 'mx']
...
for country in countries:
    ...
    data_day1 = get_appfollow_keywords(ext_id_android, date1, country, device)
    data_day2 = get_appfollow_keywords(ext_id_android, date2, country, device)
    ...
    with open(f'kw-mov-android_{country}.csv', 'w', newline='', encoding='utf-8') as file:
        writer = csv.DictWriter(file, fieldnames=['keyword', 'day1_pos', 'day2_pos', 'change'])
        writer.writeheader()
        writer.writerows(analysis_results)

    print(f"Analysis data for {country.upper()} saved to kw-mov-android_{country}.csv")
