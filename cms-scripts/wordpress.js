...
(function(){
	...
	var fillFormAndLinks = function(params){
		...
		for(var k in params){
			...
			var inputElm = document.getElementById('form-field-'+ k);
			if (inputElm) { inputElm.value = params[k]; }
		}
		...
		if(linkAttr.length > 0 && buttonsHolder.length > 0){
			...
			buttonsHolder.forEach(function(button) {
				var href = button.getElementsByTagName('a')[0].getAttribute('href');
				...
				button.getElementsByTagName('a')[0].setAttribute('href', href);
			});
